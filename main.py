...
class LLMService:
    ...
    def build_prompt(self, context_text: str, partei: str, project_context: str = "") -> str:
        ...
        return (
            ...
            f"Ihre Aufgabe:\n"
            f"Formulieren Sie eine kurze, parteitypische Stellungnahme zur laufenden Diskussion.\n\n"
            f"Zielstil:\n"
            f"Klar in der Haltung, konkret in der Forderung, sachlich im Ton. \n\n"            
            f"Verbindliche Vorgaben:\n"
            f"- Schreiben Sie in klarem, sachlichem Hochdeutsch.\n"
            f"- Vertreten Sie die Position der {partei} klar und unmissverständlich.\n"
            f"- Beginnen Sie mit einer eindeutigen Haltung, nicht mit einer neutralen Einleitung.\n"
            f"- Greifen Sie 1 bis 2 konkrete Punkte aus der Diskussion auf.\n"
            f"- Formulieren Sie eine klare Position, eine kurze Begründung und falls angemessen, eine Frage für die Diskussion.\n"
            f"- Verwenden Sie kurze, gut sprechbare Sätze.\n"
            f"- Bleiben Sie bei maximal 4 Sätzen.\n"
            f"- Klingen Sie entschlossen und politisch pointiert, aber sachlich.\n"
            f"- Vermeiden Sie vage Formulierungen wie 'müsste man prüefen', 'es bleibt abzuwarten' oder reine Ausweichantworten. \n"
            f"- Erfinden Sie keine Fakten, Zahlen oder Beschlüsse.\n"
            f"- Wenn Informationen fehlen, treffen Sie trotzdem eine klare politische Einordnung auf Basis typischer Grundhaltungen der {partei}.\n"
